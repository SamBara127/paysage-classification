...
class SceneDataset(Dataset):
    def __init__(self, samples: list[Sample], path_images: Path, transforms: Callable):
    ...
    def preprocess_samples(self, samples):
        ...
        img_list = list(map(lambda sample: self.load_sample(sample), samples))

        img_list = list(filter(lambda tensor: tensor is not None, img_list))
